...
#	liu,yu 2018-05-22 
#	V 1.0.0
#描述：部署工程pytorch-CycleGAN-and-pix2pix的docker和添加volume，步骤如下：
#1.地址：显示当前IP等信息，询问是否更改(未完成)
#2.用户：是否切换或创建用户，是则创建并切换到新用户（未完成）
#3.询问安装cpu模式或gpu模式的docker
#4.输入volume本地地址
#5.安装docker，gpu需要安装nvidia-docker
#6.编译.../CycleGAN/docker/cpuMy/Dockerfile.pytorch-CycleGAN-and-pix2pix.cpu或.../CycleGAN/docker/gpu/Dockerfile.pytorch-CycleGAN-and-pix2pix.gpu
#7.启动镜像，并加载volume
#
...
IMAGENAME=pytorch-cyclegan-and-pix2pix_dockerimage
...
do
	...
	echo "For Example:/home/$USER/OpenSourceProjects/pytorch-CycleGAN-and-pix2pix_workspace "
...
echo "############################################################################"
echo "##									##"
echo "##	检测是否有pytorch-CycleGAN-and-pix2pix_DockerImage:py27_cpu或pytorch-CycleGAN-and-pix2pix_DockerImage:py27_gpu镜像	##"
echo "##	编译.../pytorch-CycleGAN-and-pix2pix/docker/cpuMy/Dockerfile.pytorch-CycleGAN-and-pix2pix.cpu		##"
echo "##	或.../pytorch-CycleGAN-and-pix2pix/docker/gpu/Dockerfile.pytorch-CycleGAN-and-pix2pix.gpu		##"
echo "##									##"
echo "############################################################################"
...
then
	...
	then
		sudo docker build -t $IMAGENAME:$IMAGETAGCPU -f ${VOLUMEPATH}/docker/cpu/Dockerfile.pytorch-CycleGAN-and-pix2pix.cpu ${VOLUMEPATH}/docker/cpu
	else 
...
then
	...
	then
		sudo docker build -t $IMAGENAME:$IMAGETAGGPU -f ${VOLUMEPATH}/docker/gpu/Dockerfile.pytorch-CycleGAN-and-pix2pix.gpu ${VOLUMEPATH}/docker/gpu
	else 
...
echo "# 1.cd pytorch-CycleGAN-and-pix2pix/"
